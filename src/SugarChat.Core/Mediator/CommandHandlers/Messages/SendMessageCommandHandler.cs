...
namespace SugarChat.Core.Mediator.CommandHandlers.Messages
{
    public class SendMessageCommandHandler : ICommandHandler<SendMessageCommand, SugarChatResponse>
    {
        ...
        public async Task<SugarChatResponse> Handle(IReceiveContext<SendMessageCommand> context,
            CancellationToken cancellationToken)
        {
            var messageSavedEvent = await _messageService.SaveMessageAsync(context.Message, cancellationToken)
