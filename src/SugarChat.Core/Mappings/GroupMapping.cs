...
namespace SugarChat.Core.Mappings
{
    public class GroupMapping : Profile
    {
        public GroupMapping()
        {
            CreateMap<AddGroupCommand, Group>().ForMember(dest => dest.CustomPropertyList, opt => opt.Ignore());
            ...
            CreateMap<Group, GroupDto>().ForMember(dest => dest.CustomProperties, opt => opt.Ignore());
